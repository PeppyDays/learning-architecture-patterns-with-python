...
def test_cannot_allocate_if_available_smaller_than_required():
    batch, line = make_batch_and_line(
        "ELEGANT-LAMP", batch_quantity=2, line_quantity=20
    )
...
def test_can_only_deallocate_allocated_lines():
    batch, unallocated_line = make_batch_and_line(
        "DECORATIVE-TRINKET", batch_quantity=20, line_quantity=2
    )
...
def test_allocation_is_idempotent():
    batch, line = make_batch_and_line(
        "ANGULAR-DESK", batch_quantity=20, line_quantity=2
    )
    batch.allocate(line)
    batch.allocate(line)
    assert batch.available_quantity == 18
...
def test_prefers_current_stock_batches_to_shipments():
    ...
    shipment_batch = Batch(
        "batch-2", "RETRO-CLOCK", 100, eta=date.today() + timedelta(days=1)
    )
...
def test_prefers_earlier_batches():
    ...
    medium = Batch(
        "batch-2", "MINIMALIST-SPOON", 100, eta=date.today() + timedelta(days=1)
    )
    latest = Batch(
        "batch-3", "MINIMALIST-SPOON", 100, eta=date.today() + timedelta(days=2)
    )
...
def test_raises_out_of_stock_exception_if_cannot_allocate():
    batch, line = make_batch_and_line(
        "SMALL-TABLE", batch_quantity=10, line_quantity=20
    )
